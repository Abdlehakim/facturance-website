...
function formatPublishedDate(value: string, language: keyof typeof dateLocales) {
        ...
        <div className="relative overflow-hidden rounded-4xl bg-linear-to-br from-brand-deep via-brand-forest to-brand-vivid p-8 text-white shadow-[0_30px_90px_rgba(0,63,54,0.26)] sm:p-12">
          ...
          <div className="relative flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
            ...
            <div className="flex flex-wrap gap-4">
              <Link href="/contact" className="btn btn-white btn-lg shrink-0">
                {t.blog.detail.ctaPrimary}
                <ArrowRight size={20} strokeWidth={2.7} />
              </Link>
              <Link href="/pricing" className="btn btn-outline btn-lg shrink-0">
...
export function BlogArticlePageContent({ slug }: { slug: string }) {
  const { language, t } = useTranslation();
  const post = getLocalizedBlogPostBySlug(slug, language);
  ...
  return (
    <>
      <section className="relative select-none overflow-hidden border-b border-zinc-200 bg-surface-soft">
